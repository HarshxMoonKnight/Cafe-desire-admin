...
const menuImageBucket = storageConfig.menuImageBucket || "menu-images";
const menuImageFolder = storageConfig.menuImageFolder || "menu-items";
...
const menuItemImageFile = document.querySelector(".menu-item-image-file");
const uploadMenuImageButton = document.querySelector(".upload-menu-image-btn");
const menuImageUploadHint = document.querySelector(".menu-image-upload-hint");
const menuImagePreviewWrap = document.querySelector(".menu-image-preview-wrap");
...
const setMenuImageHint = (message) => {
  if (menuImageUploadHint) {
    menuImageUploadHint.textContent = message;
  }
};

const updateMenuImagePreview = (url) => {
  ...
  menuImagePreview.src = normalizedUrl;
...
const getMenuImagePath = (file) => {
  const sectionKey = normalizeMenuSectionKey(menuItemSection?.value || "misc");
  const itemName = sanitizeFileName(menuItemName?.value || file.name || "menu-item");
  const extension = file.name.includes(".") ? file.name.split(".").pop().toLowerCase() : "jpg";
  const timestamp = Date.now();

  return `${menuImageFolder}/${sectionKey || "misc"}/${timestamp}-${itemName}.${extension}`;
};

const getPublicMenuImageUrl = (path) =>
  `${SUPABASE_URL}/storage/v1/object/public/${menuImageBucket}/${path}`;
...
const uploadMenuImage = async (file) => {
  ...
  const path = getMenuImagePath(file);
  const response = await fetch(`${SUPABASE_URL}/storage/v1/object/${menuImageBucket}/${path}`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    ...
    if (apiMessage.toLowerCase().includes("bucket")) {
      throw new Error(
        `Create a public Supabase Storage bucket named "${menuImageBucket}" first, then try again.`,
      );
    }
  ...
  return getPublicMenuImageUrl(path);
};
...
const resetMenuItemForm = () => {
  ...
  setMenuImageHint("Uploads to Supabase Storage and fills the Image URL automatically.");
...
const populateMenuItemForm = (item) => {
  ...
  updateMenuImagePreview(item.image_url || "");
  setMenuImageHint("Uploads to Supabase Storage and fills the Image URL automatically.");
...
menuItemImageUrl?.addEventListener("input", () => {
  updateMenuImagePreview(menuItemImageUrl.value);
});

menuItemImageFile?.addEventListener("change", () => {
  ...
  setMenuImageHint(`Selected: ${file.name}`);
});

uploadMenuImageButton?.addEventListener("click", async () => {
  const file = menuItemImageFile?.files?.[0];
  if (!file) {
    setMessage(menuMessage, "Choose an image file first.");
    return;
  }

  uploadMenuImageButton.disabled = true;
  setMenuImageHint("Uploading image...");
  setMessage(menuMessage, "Uploading image...");

  try {
    const imageUrl = await uploadMenuImage(file);
    menuItemImageUrl.value = imageUrl;
    updateMenuImagePreview(imageUrl);
    setMenuImageHint("Image uploaded. You can save the menu item now.");
    setMessage(menuMessage, "Image uploaded.");
  } catch (error) {
    setMenuImageHint(error.message || "Image upload failed.");
    setMessage(menuMessage, error.message || "Image upload failed.");
  } finally {
    uploadMenuImageButton.disabled = false;
  }
});
